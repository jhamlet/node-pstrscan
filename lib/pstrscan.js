...
(function () {
    ...
    function scan (pattern, atPos, asStr, advance) {
        ...
        if (result && result.index <= maxPos) {
            ...
            match = atPos ? match : src.slice(pos, matchPos);
    ...
    PStrScan = PClass.derive({
        ...
        /**
         * When scanning at the current position the source string is sliced
         * by this amount to facilitate quicker scanning.
         * 
         * If the whole string was used, the regular expression used to scan
         * would have to traverse the entire string before returning false
         * if a match could not be found.
         * 
         * By setting this to a reasonable amount, we can facilitate more
         * rapid scanning.
         * 
         * This value has no effect when scanning at any position (#scanUntil,
         * #skipUntil, #checkUntil)
         * 
         * @property chunkSize
         * @type {integer}
         * @default 128
